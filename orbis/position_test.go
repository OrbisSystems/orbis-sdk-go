...
func TestClient_TransferRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := c.TransferRequest(tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("TransferRequest() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("TransferRequest() got = %v, want %v", got, tt.want)
			}
		})
	}
}
